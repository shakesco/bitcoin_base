...
final G = ECPublic.fromBytes(
    BigintUtils.toBytes(Curves.generatorSecp256k1.x, length: 32));

main() {
  final fixtures = json.decode(File('test/fixtures/silent_payments.json')
    ...
    test(testCase['comment'], () {
      ...
      for (var sendingTest in testCase['sending']) {
        ...
        for (var input in given['vin']) {
          final prevoutScript =
              Script.fromRaw(hexData: input['prevout']['scriptPubKey']['hex']);
          ...
          if (pubkey == null ||
              pubkey.getEncodeType() != EncodeType.compressed) {
            continue;
          }
        ...
        if (inputPubKeys.isNotEmpty) {
          List<SilentPaymentDestination> silentPaymentDestinations =
              (given['recipients'] as List<dynamic>)
                  .map((recipient) => SilentPaymentDestination.fromAddress(
                      recipient[0], recipient[1].floor()))
                  .toList();

          final spb = SilentPaymentBuilder(
              pubkeys: inputPubKeys, vinOutpoints: vinOutpoints);
          sendingOutputs =
              spb.createOutputs(inputPrivKeyInfos, silentPaymentDestinations);

          List<dynamic> expectedDestinations =
              sendingTest['expected']['outputs'];
          ...
          final generatedOutputs =
              sendingOutputs.values.expand((element) => element).toList();
          for (final expected in expectedDestinations) {
            ...
            expect(
                BytesUtils.toHexString(generatedPubkey.address.pubkey!
                    .toCompressedBytes()
                    .sublist(1)),
                expectedPubkey);
          }
      ...
      final aux =
          SHA256().update(utf8.encode('random auxiliary data')).digest();
      ...
      for (final receivingTest in testCase['receiving']) {
        ...
        List<String> outputsToCheck = (given['outputs'] as List<dynamic>)
        ...
        final silentPaymentOwner = SilentPaymentOwner.fromPrivateKeys(
            ...
            network: BasedUtxoNetwork.values[1]);
        ...
        for (var label in given['labels']) {
          receivingAddresses
              .add(silentPaymentOwner.toLabeledSilentPaymentAddress(label));
          final generatedLabel = silentPaymentOwner.generateLabel(label);

          preComputedLabels ??= {};
          preComputedLabels[G
              .tweakMul(BigintUtils.fromBytes(generatedLabel))
              .toHex()] = BytesUtils.toHexString(generatedLabel);
        }

        for (var address in expected['addresses']) {
          expect(
              receivingAddresses
                  .indexWhere((sp) => sp.toString() == address.toString()),
              isNot(-1));
        }
        ...
        if (inputPubKeys.isNotEmpty) {
          ...
          for (int i = 0; i < expectedDestinations.length; i++) {
            ...
            var fullPrivateKey = silentPaymentOwner.b_spend
                .tweakAdd(BigintUtils.parse(privKeyTweak));
            ...
            final btcSigner =
                BitcoinSigner.fromKeyBytes(fullPrivateKey.toBytes());
            List<int> sig = btcSigner.signSchnorrTransaction(msg,
                tapScripts: [], tweak: false, aux: aux);

            // Verify the message is correct
            expect(btcSigner.verifyKey.verifySchnorr(msg, sig, isTweak: false),
                true);

            // Verify the signature is correct
            expect(BytesUtils.toHexString(sig),
